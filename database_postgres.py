...
#       enroll.py imports THIS file as "db", so all fixes go here.
#
# FIXES v8.5:
#  - add_student() now accepts gender=, mobile_number=, department=,
#    email=, face_encoding_path=, dataset_folder= (enroll.py v11 compat)
#  - Added update_student_gender() — was completely missing
#  - All SQL uses ? placeholders (SQLite), zero %s in queries
#  - All functions present for api.py, enroll.py, recognizer.py, main.py
# =============================================================
...
def init_db():
    with db() as conn:
        ...
        # Migrate: safely add new columns to any existing DB
...
def _safe_add_column(conn, table, column, definition):
    """Add column only if it does not already exist — safe for upgrades."""
    try:
...
def get_student(student_id: str):
    """Return a single active student as dict, or None."""
    with db() as conn:
        row = conn.execute(
...
def add_student(student_id,
                ...
                roll_number,
                section            = "",
                mobile             = "",        # original param
                mobile_number      = "",        # alias used by some callers
                twin_of            = None,
                consent            = True,
                gender             = "",        # enroll.py v11 passes this
                department         = "",
                email              = None,
                face_encoding_path = "",
                dataset_folder     = "") -> bool:
    ...
    Compatible with old style:
        db.add_student(sid, name, roll, mobile=x, section=y, twin_of=z)
    And new style (enroll.py v11):
        db.add_student(sid, name, roll, mobile=x, section=y, twin_of=z, gender=g)

    Returns True on success, False if roll_number already exists.
    """
    mob = mobile_number or mobile   # accept either keyword
    try:
        with db() as conn:
            conn.execute("""
            ...
            # Save gender into the migrated column (non-fatal if missing)
            if gender:
                try:
                    conn.execute(
        ...
        return False  # duplicate roll_number


def update_student_gender(student_id: str, gender: str):
    """
    Update gender for an existing student.
    Called by enroll.py on re-enrolment (student already in DB).
    Uses SQLite ? placeholders — NOT PostgreSQL %s syntax.
    """
    try:
        with db() as conn:
            conn.execute(
...
def delete_student_data(student_id: str):
    """Soft-delete: sets active=0, all data is retained."""
    with db() as conn:
        conn.execute(
...
def register_twin_pair(id1: str, id2: str):
    """Register a mutual twin relationship between two students."""
    with db() as conn:
...
def log_twin_analysis(student_id, name="", twin_id="",
                      ...
                      feature_vector="",
                      twin_partner_id=None):   # legacy alias for twin_id
    """
    Log twin disambiguation result.
    twin_partner_id is a backward-compatible alias for twin_id.
    """
...
# =============================================================
# STARTUP — runs once on import
# =============================================================
try:
    init_db()
    print(f"  DB connected — {len(get_all_students())} student(s)")
