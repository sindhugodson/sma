...
class _ConfirmBuffer:
    ...
    def get(self):
        ...
        valid = [x for x in self._ids if x]
        if len(valid) < req:
            return None, 0.0
...
class SmartRecognizer:
    ...
    def _run_lbph(self, gray):
        """
        v9.3 FIX: equalizeHist is variant[0] — matches training.
        Expected distances after retraining:
          Good match:  dist  5-30  → conf 70-95%
          OK match:    dist 30-60  → conf 40-70%
          Weak match:  dist 60-100 → conf 10-40%
          Reject:      dist > 120
        """
        ...
        if DEBUG:
            name   = self.lbph_labels.get(best_label, "?")
            status = ("UNK_CLASS"  if name == "__UNKNOWN__" else
                      "EXCELLENT"  if best_dist < 20 else
                      "GOOD"       if best_dist < 40 else
                      "PASS"       if best_dist < margin else
                      "WEAK"       if best_dist < thr else
                      "REJECT")
            print(f"  [LBPH] {name}: dist={best_dist:.1f} "
                  f"margin={margin} thr={thr} → {status}")

        if best_label is None or best_dist >= thr:
        ...
        # Confidence: 100% at dist=0, 0% at dist=margin
        # Above margin and below thr: weak pass (10-35%)
        if best_dist < margin:
            conf = float(np.clip(1.0 - best_dist / margin, 0.0, 1.0))
        else:
            span = max(thr - margin, 1)
            conf = float(np.clip(
                0.35 * (1.0 - (best_dist - margin) / span),
                0.10, 0.34))
